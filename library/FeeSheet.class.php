...
use OpenEMR\Services\Inventory\ProcedureProductService;
...
class FeeSheet
{
  ...
  //
    ...
    protected function applyProcedureProductConsumption(array $billItems, $defaultWarehouse = null): void
    {
        $service = new ProcedureProductService();
        $requirements = $service->buildRequirementsFromBill($billItems);

        if (empty($requirements)) {
            try {
                $this->clearProcedureProductSales();
            } catch (\Throwable $e) {
                error_log('Failed clearing procedure-product inventory reservations: ' . $e->getMessage());
            }
        ...
        if ($defaultWarehouse === null) {
            $defaultWarehouse = $this->default_warehouse;
        }

        $defaultWarehouse = $defaultWarehouse ?? '';
        $failedReservations = [];
        foreach ($requirements as $mappingId => $requirement) {
            $expiredLots = null;
            $ok = sellDrug(
                $requirement['drug_id'],
            ...
            if (!$ok) {
                $failedReservations[] = $requirement + ['mapping_id' => $mappingId];
            }
        }

        if (!empty($failedReservations)) {
            error_log('Procedure product sync skipped due to inventory shortage: ' . json_encode($failedReservations));
            return;
        }

        \sqlBeginTrans();
        try {
            $this->clearProcedureProductSales(false);
            foreach ($requirements as $mappingId => $requirement) {
                $expiredLots = null;
                $saleId = sellDrug(
                    $requirement['drug_id'],
                ...
                if (empty($saleId)) {
                    throw new \RuntimeException('Inventory allocation failed for procedure mapping ' . $mappingId);
                }
            }
            \sqlCommitTrans();
        } catch (\Throwable $e) {
            \sqlRollbackTrans();
            error_log('Procedure product synchronization failed: ' . $e->getMessage());
        }
    }

    protected function clearProcedureProductSales(bool $manageTransaction = true): void
    {
        ...
        try {
            $existing = \sqlStatement(
                "SELECT sale_id, inventory_id, quantity FROM drug_sales WHERE pid = ? AND encounter = ? AND selector LIKE 'AUTO_PROC:%'",
                array($this->pid, $this->encounter)
            );
            while ($row = \sqlFetchArray($existing)) {
                if (!empty($row['inventory_id'])) {
                    \sqlStatement(
                        "UPDATE drug_inventory SET on_hand = on_hand + ? WHERE inventory_id = ?",
                        array($row['quantity'], $row['inventory_id'])
                    );
                }
                \sqlStatement("DELETE FROM drug_sales WHERE sale_id = ?", array($row['sale_id']));
            }
  ...
  //
    public function save(
        ...
        $this->applyProcedureProductConsumption($bill ?? [], $default_warehouse);
